...
import {
  ChevronDown, ChevronUp, Save, X, MapPin, Mail, Upload
} from 'lucide-react';
    ...
    <div className="w-full space-y-4">
      {user?.department && user.role !== 'admin' && (
        <div className="text-sm bg-blue-50 border border-blue-100 px-4 py-2.5 rounded-xl">
          Showing leads for department: <strong>{user.department}</strong>
        </div>
      )}
      {/* Mobile View */}
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              {/* <span className="px-2 py-1 text-xs rounded-lg bg-gray-100 text-gray-700 font-semibold">
                {lead.course || "N/A"}
              </span> */}
            </div>
          ...
          <table className="w-full min-w-[1800px] table-fixed divide-y divide-slate-100">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {filteredData.map((lead) => {
                ...
                return (
                  <>
                    <tr key={lead._id} className={`transition-colors duration-200 hover:bg-slate-50/80 ${isEditing ? 'bg-orange-50/10' : ''}`}>
                      {/* Enquiry ID */}
