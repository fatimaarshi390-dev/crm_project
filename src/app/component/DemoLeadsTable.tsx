...
export default function DemoDayLeadsTable() {
  ...
  // ── Fetch courses + teachers when dept available ──
  useEffect(() => {
    if (user?.department) {
  ...
  if (loading)
    return <p className="text-center py-10 text-gray-500">Loading Demo Day leads...</p>;
      ...
      <table className="w-full min-w-[2000px] border-collapse">
        <thead className="sticky top-0 z-10 bg-gradient-to-r from-blue-900 to-blue-700 text-white">
          <tr>
            ...
            <th className="px-5 py-4 text-left">POST-DEMO DATE</th>
        ...
        <tbody>
          {leads.map((lead) => {
            ...
            const historyDates = lead.demoDateHistory ||
                ...
                <tr key={lead._id} className="border-b hover:bg-blue-50 transition-all">
                  ...
                  <td className="px-5 py-4 text-center">
                    {isEditing ? (
                      <input
                        type="checkbox"
                        checked={editForm.demoDone}
                        onChange={(e) => setEditForm({ ...editForm, demoDone: e.target.checked })}
                  ...
                  <td className="px-5 py-4">
                    {isEditing ? (
                      <select
                        ...
                        onChange={(e) => setEditForm({ ...editForm, demoDoneBy: e.target.value })}
                        className="border rounded px-3 py-1 w-full"
                      >
                  ...
                  <td className="px-5 py-4 font-medium">
                    {isEditing ? (
                      <input
                        ...
                        onChange={(e) => setEditForm({ ...editForm, postDemoDate: e.target.value })}
                        className="border rounded px-3 py-1 w-full"
                      />
                    ) : (
            ...
            {isEditing ? (
              ...
              <span className="px-3 py-1 rounded-xl text-xs font-bold bg-slate-100">
                {lead.interested || "Not Set"}
                  ...
                  {/* Reminder */}
                  <td className="px-5 py-4">{lead.reminder || 'No'}</td>
